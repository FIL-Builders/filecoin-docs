...
Note that MetaMask supports Filecoin EVM addresses that follow the Ethereum `0x` format (see [this section](https://docs.filecoin.io/networks-and-tools/assets/transfer-fil) for more info on address types). To use native Filecoin address types that begin with `f`, you can use:
